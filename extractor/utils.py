...
def chunk_page_text(page):
    ...
    If a heading is not clearly detected, groups paragraphs under "Untitled".
    """
    ...
    for block in blocks:
        ...
        # Heuristic: large + bold text as title
        ...
        if title_size > 11 and bold_font and len(title_candidate.split()) < 12:
            ...
            title = "Untitled"
            content = " ".join([clean_text(span["text"]) for span in spans])

        if content.strip():  # Avoid adding empty blocks
